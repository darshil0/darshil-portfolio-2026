import { expect, test } from "vitest";
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import * as data from "../src/constants/data";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

test("index.html has correct version and experience", () => {
  const content = fs.readFileSync(
    path.resolve(__dirname, "../index.html"),
    "utf8",
  );
  expect(content).toContain("v2026.8.1");
  expect(content).toContain("14+ years");
});

test("src/App.jsx uses centralized version and experience", () => {
  const content = fs.readFileSync(
    path.resolve(__dirname, "../src/App.jsx"),
    "utf8",
  );
  expect(content).toContain("portfolioVersion");
  expect(content).toContain("experienceYears");
});

test("constants have correct values", () => {
  expect(data.portfolioVersion).toBe("v2026.8.1");
  expect(data.experienceYears).toBe("14+ years");
});

test('index.html has lang="en"', () => {
  const content = fs.readFileSync(
    path.resolve(__dirname, "../index.html"),
    "utf8",
  );
  expect(content).toContain('html lang="en"');
});

test("index.html has legacy-main-content", () => {
  const content = fs.readFileSync(
    path.resolve(__dirname, "../index.html"),
    "utf8",
  );
  expect(content).toContain('id="legacy-main-content"');
});

test("metrics are centralized and correct", () => {
  expect(data.metrics).toBeDefined();
  expect(data.metrics.defectEscapeRate).toBe("< 1%");
  expect(data.metrics.automationReliability).toBe("99.5%");
  expect(data.metrics.waitTimeReduction).toBe("30%");
  expect(data.metrics.testCoverageGrowth).toBe("25%");
});
