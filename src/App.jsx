...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Manage scroll progress (Bug 5 Fix native)
      const winScroll = document.body.scrollTop || document.documentElement.scrollTop;
      const height = document.documentElement.scrollHeight - document.documentElement.clientHeight;
      const scrolled = height > 0 ? (winScroll / height) * 100 : 0;
      setScrollProgress(scrolled);

      // Check which section is in view to update active tab natively
      const sections = ['home', ...navItems.map(n => n.id), 'contact'];
      for (let i = sections.length - 1; i >= 0; i--) {
        const el = document.getElementById(`${sections[i]}-section`);
        if (el) {
          const rect = el.getBoundingClientRect();
          // if top is near top edge, or we have scrolled past its top edge
          if (rect.top <= 100) {
