  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border-b border-slate-200/40 dark:border-slate-800/40 shadow-sm transition-all duration-300">
      ...
      {isMenuOpen && (
        <div
          id="mobile-menu"
          className="fixed inset-0 top-16 bg-white dark:bg-slate-900 z-[60] p-6 flex flex-col space-y-4 overflow-y-auto lg:hidden"
        >
          <button
            id="nav-home-m"
            onClick={() => navigate("home")}
            className={`text-left py-2 text-lg font-semibold ${activeSection === "home" ? "text-[#00685f] dark:text-[#6bd8cb]" : ""}`}
          >
            Home
          </button>
          {navItems.map((item) => (
          ...
          <button
            id="nav-contact-m"
            onClick={() => navigate("contact")}
            className={`text-left py-2 text-lg font-semibold ${activeSection === "contact" ? "text-[#00685f] dark:text-[#6bd8cb] font-black" : "text-slate-800 dark:text-white"}`}
          >
            Contact
          </button>
        </div>
