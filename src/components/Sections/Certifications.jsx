    ...
    <section id="certifications-section" className="py-20 bg-[#f8f9ff] dark:bg-slate-800 min-h-[600px]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
          {certs.map((cert, i) => (
            <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm hover:border-[#00685f] transition-colors break-inside-avoid">
              ...
              <p className="text-xs text-slate-500 font-medium">{cert.issue}{cert.date ? ` · ${cert.date}` : ''}</p>
            </div>
          ))}
        </div>
      </div>
