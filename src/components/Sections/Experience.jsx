...
const experiences = [
  ...
  },
  {
    ...
    company: "Tata Consultancy Services (Sony Pictures)",
...
export default function Experience() {
  return (
    <section
      ...
      className="py-24 bg-[#f8f9ff] dark:bg-[#0b111a] min-h-[600px] relative overflow-hidden"
    >
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="mb-20">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-lg font-medium italic opacity-80">
            {experienceYears} of high-impact testing leadership across
            healthcare, entertainment & pharma.
          </p>
        </div>
        ...
        <div className="space-y-12">
          {experiences.map((exp, i) => (
            <div
              key={i}
              className="group relative bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 shadow-2xl border border-slate-100 dark:border-slate-800 transition-all duration-500 hover:shadow-emerald-900/5"
            >
              <div className="flex flex-col lg:flex-row gap-10">
                <div className="lg:w-1/3">
                  <div className="flex items-start gap-5 mb-6">
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#00685f] to-[#004e47] flex items-center justify-center text-white font-black text-sm shadow-xl shrink-0 group-hover:scale-110 transition-transform duration-500">
                      {exp.code}
                    </div>
                    <div>
                      <h3 className="text-2xl font-black font-headline tracking-tight text-slate-800 dark:text-white leading-tight mb-2">
                        {exp.role}
                      </h3>
                      <p className="text-[#00685f] dark:text-[#6bd8cb] font-black text-sm uppercase tracking-widest">
                        {exp.company}
                      </p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <p className="text-slate-400 dark:text-slate-500 text-xs font-bold uppercase tracking-widest ml-1">
                      {exp.location}
                    </p>
                  </div>
                </div>
                ...
                <div className="lg:w-2/3 lg:border-l lg:border-slate-100 lg:dark:border-slate-800 lg:pl-10">
                  <ul className="grid gap-5">
                    {exp.bullets.map((b, idx) => (
                      <li
                        key={idx}
                        className="flex items-start gap-4 text-slate-600 dark:text-slate-300 text-sm leading-relaxed group/item transition-all"
                      >
