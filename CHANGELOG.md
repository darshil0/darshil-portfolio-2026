...
- **HTML**: Corrected heading hierarchy in `experience.html` (changed `<h4>` to `<h3>`) to meet WCAG accessibility standards.
- **CSS**: Replaced hardcoded `white` background with `var(--surface)` design token in mobile navigation.
- **Changelog**: Fixed spurious extra blank line in version `2026.3.1` metadata.
